    ...
    >
      {asset ? (
        <div className="absolute inset-0 -z-10" aria-hidden>
          ...
          <div className="absolute inset-0 bg-[linear-gradient(180deg,rgb(var(--surface-rgb)/0.62),rgb(var(--surface-rgb)/0.3)_40%,var(--color-surface)_84%)]" />
        </div>
      ) : null}
      <div className="mx-auto w-full max-w-[1200px] px-6 md:px-10">
        <h1 className="max-w-[18ch] text-balance text-4xl font-medium leading-[1.05] tracking-tight text-fg md:text-6xl">
          {title}
        </h1>
        {intro ? (
          <p className="mt-5 max-w-[58ch] text-lg leading-relaxed text-fg/80">{intro}</p>
        ) : null}
        {children}
      </div>
    </section>
